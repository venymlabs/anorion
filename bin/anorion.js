...
function getAnorionDir() {
  ...
  if (existsSync(resolve(cwd, "anorion.yaml")))
    return cwd;
  let dir = cwd;
  for (let i = 0;i < 10; i++) {
    ...
    if (parent === dir)
      break;
    if (existsSync(resolve(parent, "anorion.yaml")))
      return parent;
    dir = parent;
  }
  return cwd;
...
function green(text) {
  return import_picocolors.default.green(text);
}
function red(text) {
  return import_picocolors.default.red(text);
}
function yellow(text) {
  return import_picocolors.default.yellow(text);
}
function cyan(text) {
  return import_picocolors.default.cyan(text);
}
function magenta(text) {
  return import_picocolors.default.magenta(text);
}
function header(text) {
  const line = "─".repeat(52);
  console.log(`
${import_picocolors.default.red("\uD83D\uDD25")} ${bold(text)}
${dim(line)}`);
}
function error(msg) {
  console.error(`
...
`));
  const { prompt: prompt2, confirm: confirm2, choose: choose2 } = await Promise.resolve().then(() => exports_interactive);
  const { writeFileSync, mkdirSync } = await import("node:fs");
  const { stringify: stringifyYaml } = await Promise.resolve().then(() => __toESM(require_dist(), 1));
  const config = {};
  config.gateway = {
    host: await prompt2("Host", "0.0.0.0"),
    port: parseInt(await prompt2("Port", "4250")),
...
Available LLM providers:`);
  ...
  const pIdx = await choose2("Select primary LLM provider:", providers);
  ...
  let model;
  if (models.length > 1) {
    const mIdx = await choose2("Select model:", models);
    model = models[mIdx] || defaultModels[providerId];
  } else {
    model = models[0] || defaultModels[providerId];
  }
  const fullModel = `${providerId}/${model}`;
  const agentName = await prompt2("Agent name", "assistant");
  const agentPrompt = await prompt2("System prompt", "You are a helpful AI assistant with access to tools. Be concise and direct.");
  let telegram = { enabled: false };
  if (await confirm2("Enable Telegram channel?", false)) {
    telegram = {
      enabled: true,
      botToken: await prompt2("Telegram bot token"),
      allowedUsers: (await prompt2("Allowed user IDs (comma-separated)")).split(",").map((s) => s.trim()).filter(Boolean),
...
async function startDaemon(anorionDir) {
  ...
  try {
    ...
    try {
      ...
      if (metricsRes.ok) {
        ...
        if (memMatch)
          console.log(`  ${dim("Memory:")}    ${formatBytes(parseInt(memMatch[1]))}`);
      }
    } catch {}
    try {
      ...
      if (chRes.ok) {
        ...
        if (channels.length > 0) {
          console.log(`  ${dim("Channels:")}  ${channels.map((c) => c.name).join(", ")}`);
        }
      }
    } catch {}
  } catch {
...
async function cmdConfig() {
  ...
  switch (subcmd) {
    case "get": {
      const key = args[2];
      if (!key) {
        error("Usage: anorion config get <key>");
        return;
      }
      const config = loadYamlConfig();
      if (!config) {
    ...
    case "set": {
      ...
      if (!key || value === undefined) {
        error("Usage: anorion config set <key> <value>");
        return;
      }
      const config = loadYamlConfig();
      if (!config) {
      ...
      const { stringify: stringifyYaml, writeFileSync } = await Promise.resolve().then(() => __toESM(require_dist(), 1));
...
async function cmdAgent() {
  ...
  switch (subcmd) {
    ...
    case "create": {
      const { prompt: prompt2, choose: choose2 } = await Promise.resolve().then(() => exports_interactive);
      const { writeFileSync, mkdirSync } = await import("node:fs");
      const { stringify: stringifyYaml } = await Promise.resolve().then(() => __toESM(require_dist(), 1));
      const name = await prompt2("Agent name");
      if (!name) {
      ...
      const mIdx = await choose2("Model:", modelOptions);
      const model = modelOptions[mIdx] || "openai/gpt-4o";
      const systemPrompt = await prompt2("System prompt", "You are a helpful assistant.");
    ...
    case "show": {
      const id = args[2];
      if (!id) {
        error("Usage: anorion agent show <id>");
        return;
      }
      try {
    ...
    case "delete": {
      const id = args[2];
      if (!id) {
        error("Usage: anorion agent delete <id>");
        return;
      }
      try {
...
async function cmdChannel() {
  ...
  switch (subcmd) {
    ...
    case "enable": {
      const name = args[2];
      if (!name) {
        error("Usage: anorion channel enable <name>");
        return;
      }
      try {
    ...
    case "disable": {
      const name = args[2];
      if (!name) {
        error("Usage: anorion channel disable <name>");
        return;
      }
      try {
...
async function cmdTool() {
  ...
  switch (subcmd) {
    ...
    case "exec": {
      ...
      if (!toolName) {
        error("Usage: anorion tool exec <name> [args]");
        return;
      }
...
async function cmdLogs() {
  ...
  if (!logFile || !existsSync(logFile)) {
    ...
    if (existsSync(logDir)) {
      ...
      if (files.length > 0)
        logFile = resolve(logDir, files[0]);
    }
  }
  if (!logFile || !existsSync(logFile)) {
...
async function cmdDoctor() {
  ...
  try {
    ...
    if (bunVer)
      success(`Bun ${bunVer}, Node ${nodeVer}`);
    else
      success(`Node ${nodeVer} (Bun not found)`);
  } catch {
...
function loadYamlConfig() {
  ...
  if (!existsSync(configPath))
    return null;
  try {
    const { parse: parseYaml } = require_dist();
